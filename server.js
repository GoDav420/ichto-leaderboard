...
function getTrimmedAverage(scores, leniency = 1.0) {
    if (!scores || scores.length === 0) return 0;
    
    let adjusted = scores.map(s => s / leniency);
    adjusted.sort((a, b) => a - b);
    
    if (adjusted.length >= 3) {
        adjusted = adjusted.slice(1, -1);
    }
    
    const sum = adjusted.reduce((a, b) => a + b, 0);
    return adjusted.length > 0 ? sum / adjusted.length : 0;
}
...
function fallbackCalculation(teams, sections) {
    ...
    teamsArray.forEach(team => {
        ...
        if (!sectionGroups[sectionId]) {
            sectionGroups[sectionId] = {
                sectionId,
                sectionName: sections[sectionId]?.name || sectionId,
                teams: [],
                allScores: [],
                meanRaw: 0,
                stdRaw: 0,
        ...
        grades.forEach(g => {
            ...
            teamScores.push(...scores);
            sectionGroups[sectionId].allScores.push(...scores);
        });
    ...
    Object.values(sectionGroups).forEach(section => {
        if (section.allScores.length > 0) {
            ...
            const variance = section.allScores.reduce((acc, s) => 
                acc + Math.pow(s - section.meanRaw, 2), 0);
            section.stdRaw = Math.sqrt(variance / section.allScores.length);
        }
    });
    ...
    // Calculate leniency for each section
    Object.values(sectionGroups).forEach(section => {
        if (globalMean > 0 && section.meanRaw > 0) {
            section.leniency = section.meanRaw / globalMean;
        }
    ...
    // Calculate global TP statistics
    const tpSum = results.reduce((acc, t) => acc + t.tp, 0);
    const globalMeanTp = results.length > 0 ? tpSum / results.length : 0;
    
    const tpVariance = results.reduce((acc, t) => 
        acc + Math.pow(t.tp - globalMeanTp, 2), 0);
...
async function calculateScores(teams, sections) {
    return new Promise((resolve) => {
        ...
        try {
            ...
            teamsArray.forEach(team => {
                const sectionId = team.section || 'default';
                const sectionName = sections[sectionId]?.name || sectionId;
                const grades = team.grades || [];
...
app.post('/api/teams', requireAdmin, async (req, res) => {
    try {
        ...
        if (!name || !nationality) {
            return res.status(400).json({ error: 'Name and nationality required' });
        }
...
app.post('/api/grades/upload', requireAdmin, async (req, res) => {
    try {
        ...
        if (!gradesString || !gradesString.trim()) {
            return res.status(400).json({ error: 'Grades string required' });
        }
        ...
        teamGrades.forEach(teamGradeStr => {
            ...
            // Parse section|team|country format
            if (teamIdentifier.includes('|')) {
                ...
                if (parts.length === 3) {
                    ...
                    let foundSection = Object.entries(data.sections)
                        .find(([id, s]) => s.name === sectionName);
                    if (!foundSection) {
                        sectionId = uuidv4();
                    ...
                    // Find or create team
                    let foundTeam = Object.entries(data.teams)
                        .find(([id, t]) => t.name === teamName);
                    if (!foundTeam) {
                        teamId = uuidv4();
                        data.teams[teamId] = {
                            id: teamId,
...
server.listen(PORT, () => {
    ...
    console.log('(Leniency-Adjusted Scoring System)');
    console.log('========================================');
